...
DISCORD_WEBHOOK_URL = "Wklej_tutaj_twój_webhook_do_kanalu_ciekawostka-matematyczna"

# LISTA 365 ciekawostek matematycznych (krótkie, w języku polskim)
FACTS = [
...
"Zastosowania transformacji Laplace'a i Fouriera w inżynierii i przetwarzaniu sygnałów.",
"Źródła: klasyczne problemy i ciekawostki matematyczne — świetne punkty do dalszego zgłębiania.",
]
...
def get_index_for_day(day_of_year=None):
    ...
    # map 1..365 (ignore leap-year rollover: use modulo to be safe)
        ...
        "embeds": [
            {
                ...
                "color": 3447003,
                "footer": {
                    "text": "Codzienna ciekawostka matematyczna • Bot"
                },
...
def main():
    # opcjonalny argument: numer dnia (1..365)
    day_arg = None
    if len(sys.argv) > 1:
        try:
    ...
    fact = FACTS[idx]
    title = f"📏 Codzienna ciekawostka matematyczna — dzień {idx+1}"
